...
LDLIBS=`sdl-config --libs` `pkg-config --libs glew gl SDL_image libavcodec libavformat libavutil libswscale` -lm

#CVT
#CFLAGS=-O2 -pipe -pedantic-errors -Wall -Wextra -march=native -fomit-frame-pointer -std=gnu99 `sdl-config --cflags`
#LDFLAGS=-L/usr/lib
#LDLIBS=`sdl-config --libs` -lSDL_image -lGL -lGLEW -lavcodec -lavformat -lavutil -lswscale  -lm
